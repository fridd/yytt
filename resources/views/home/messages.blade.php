        ...
        <div class="about-content">
            ...
            <div class="about-txt messages">
                ...
                <form action="{{ isset($model)?'':url('/messages/info') }}" method="POST" id="messagesForm">
                    ...
                    <div class="me-form-row">
                        <label>留言类型</label>
                        <input type="text" value="{{ old('message_type_id',  $model->message_type_id) }}">
                    </div>                    
                    @endif
                    <div class="me-form-row">
                        <label>姓名</label>
                        <input type="text" name="name" class="fname" placeholder="请输入您的姓名" required="required" value="{{ isset($model) ? $model->name : old('name') }}">
                    </div>
                    <div class="me-form-row">
                        <label>联系方式</label>
                        <input type="text" name="tel" class="ftel" placeholder="请输入您的联系电话" required="required" value="{{ isset($model) ? $model->tel : old('tel') }}">
                    </div>                    
                    <div class="me-form-row">
                        <label>邮箱</label>
                        <input type="email" name="email" class="femail" placeholder="请输入您的邮箱" required="required" value="{{ isset($model) ? $model->email : old('email') }}">
                    </div>
                    ...
                    <div class="me-form-row">
                        <label>地址</label>
                        <input type="text" name="address" class="faddress" placeholder="请输入您的地址" required="required" value="{{ isset($model) ? $model->address : old('address') }}">
                    </div>  
                    <div class="me-form-row cont-row">
                        <label>留言内容</label>
                        <textarea placeholder="请输入您的留言内容" name="content" class="fcontent" required="required">{{ isset($model) ? $model->content : old('content') }}</textarea>
                    </div>
                    @if (!isset($model))
                    <div class="me-form-row">
                        <label>验证码</label>
